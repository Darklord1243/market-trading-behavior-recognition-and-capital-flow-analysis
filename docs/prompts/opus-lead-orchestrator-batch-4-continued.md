# Opus lead orchestrator — Batch 4 continued (post–B.3c DEFERRED → H.2 ✅ → P3.1)
...
| LIS state | v1.6.3 (`f68527b`) — B.3c DEFERRED; **H.2 docs sync committed on top** |
...
## Next — P3.1 (Phase 3 first slice; Opus scopes FIRST; Sonnet blocked until prompt approved)

**Critical constraint:** LIS § Phase 3 batch 3a (OFI) alone adds features to the matrix but does **not** wire into
`rules.py`. Feature-only slices will not beat 0.6449. **P3.1 must move the scorer, not just widen the feature matrix.**

You do (Opus, before any Sonnet dispatch):
1. Run `--verbose-scores` on all current misclassifications (5 keys above + any others on n=39).
2. Read LIS § Phase 3 (batches 3a–3e) and Feature B § (B.1 optional, B.3c deferred routing).
3. Propose the **smallest** slice that: can plausibly beat **0.6449/n=39**; holds **n=24 continuity ≥ 0.6599**;
   targets deferred 散户 feature-coverage gaps where feasible; uses **global constants only** (no label grid-search);
   wires into `DIMS_RETAIL` / `rules.py` if scoring impact is required.
4. If no slice plausibly beats 0.6449, **say so** and recommend V.3.3 labels or a narrower diagnostic — do not
   dispatch Sonnet on a known-flat path.
5. Draft the full Sonnet prompt (save to `docs/prompts/sonnet-phase-3-p3.1-<slug>.md` or paste inline).
6. **Stop** — present scope rationale + prompt for human approval. Do not dispatch Sonnet yet.

Sonnet dispatch (only after human says "approve prompt / dispatch P3.1"):
...
Then run full DISPATCH → … → GATE. Two commits if needed (feat + LIS v1.6.4) per the B.2 pattern.
...
- Do not dispatch P3.1 Sonnet before the human approves your drafted prompt.
