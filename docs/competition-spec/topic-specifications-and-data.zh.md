...
> **运营说明（A榜）：** 平台 FAQ 曾补充 **18:00 更新答案 → 次日 08:00 前**上传可得即时分。  
> **B榜勿用该窗口** — 见 [`../official_guidance/b-board-rules.zh.md`](../official_guidance/b-board-rules.zh.md)。
...
| 规则 | 说明 |
|------|------|
| 时间 | 2026/07/13 — 2026/07/24 17:00 |
| 底层数据 | 2026/07/10 — 2026/07/23 |
| 提交 | 每个交易日**必须提交一次**；单日最多 **3** 次 |
| 提交窗口 | **T+1 15:00 – T+2 14:59**（逾期该日得 0） |
| 最低天数 | **不满 8 个交易日 → 不计入最终排名** |
| 单日取分 | 当日**最高分**（非最后一次） |
| 评分 | **9 日 WMA**（权重 9…1，分母 45）；约 T+5 更新 |
| 评测截止 | **7/24 15:00**；末评测交易日 **7/22**；**7/24 不再更新股票样本** |
| 公布 | 拟 **2026/07/28** |
| 审核 | **TOP15** 结果复现审核，决定决赛路演资格 |

> **权威细则（窗口示例、WMA 公式、与 A 榜差异）：** [`../official_guidance/b-board-rules.zh.md`](../official_guidance/b-board-rules.zh.md)。上表为摘要；冲突时以该文件为准。
